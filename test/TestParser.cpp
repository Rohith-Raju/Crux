...
TEST(ParserCheck, TestParserBasic){
    ...
    Parser p(tokens);
    std::shared_ptr<Expr> expression  = p.parse();
...
TEST(ParserCheck, TestParserComplex){
    ...
    Parser p(tokens);
    std::shared_ptr<Expr> expression  = p.parse();
    ASSERT_EQ(AstPrinter{}.print(expression), "(+ (+ 10.000000 (Group (- (+ 40.000000 (Group (- 20.000000 30.000000))) 10.000000))) 50.000000)");
}
...
TEST(ParserCheck, TestParserComplexUnary){
    ...
    Parser p(tokens);
    std::shared_ptr<Expr> expression  = p.parse();
    ASSERT_EQ(AstPrinter{}.print(expression), "(+ (+ (! true) (Group (- 30.000000 false))) 40.000000)");
}
